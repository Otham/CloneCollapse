	SELECT DISTINCT a.employee_id, a.client_roster_id, a.office_id, 
		...
		ISNULL(eb.bonus, 0.00) as bonus,  ISNULL(cc.cost_center_id,0) as cost_center, 
		ISNULL(cc.cost_center_id,0) as cost_center_id,
		@modify AS modify
	...
	FROM client_roster a 
		...
		INNER JOIN user_department h on b.department_id=h.department_id and
		h.client_membership_id=@clientMembershipId
		...
		LEFT OUTER JOIN employee_bonus eb on eb.aident_number = c.aident_number AND eb.week_ending = @weekEndDate 
		AND eb.client_id = @clientID AND eb.dept_id = b.department_id
